...
- [Review setup](references/setup.md) — explicit setup request triggers, setup proposal template, pre-approval no-mutation rule, post-approval repository capability detection, read-only CI inspection policy, setup plan output, approved normalized review command application, stable setup documentation contract, setup verification classification, allowed/forbidden setup changes, and setup verification checklist.
...
3. **Setup intake first.** Before setup inspection or mutation begins, propose a setup scope in chat and ask for go/no-go. The proposal must name target repository, planned inspection, possible file/doc/command changes, forbidden production/CI changes, expected output artifacts, and the approval gate. Do not install tooling, add commands, edit docs, touch CI, or make other mutations before approval. After setup-scope approval, run bounded repository capability detection including read-only CI status detection, and present a concrete setup plan. Apply setup changes only after the user approves that plan; approved normalized review command setup must use the precedence `existing Just -> existing Make -> existing npm/package scripts -> add Just if no runner exists and the user approved it`, cover sizing/static/dependency-architecture/tests/coverage/aggregate review where appropriate, record target-repository CI status and policy summary in setup documentation, preserve the no-production-dependency/no-CI-mutation/no-new-merge-blocker boundary, and verify each safe configured command with per-command classifications that distinguish diagnostic findings from broken or unavailable tooling.
...
2. **Setup path:** for setup requests, load [Review setup](references/setup.md), propose setup scope, and wait for go/no-go before any inspection or mutation. After setup-scope approval, perform bounded capability detection including read-only CI status detection, produce a setup plan, and ask for the next setup decision. If the user approves the setup plan, apply only the approved repo-local/dev documentation, tooling, or normalized command changes; never change production dependencies, production config, CI, required checks, branch protections, merge blockers, deployment, or secrets as part of this setup workflow. Verify safe configured commands, classify each result, keep the chat summary concise, and stop unless the user starts a separate review workflow.
