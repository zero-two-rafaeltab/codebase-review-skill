...
After the user approves the setup scope, run the bounded capability detection and setup-plan step from [Review setup](setup.md) before applying any changes. Include read-only CI status detection when it is part of the approved lightweight inspection: if no CI exists, say so and do not create CI; if CI exists, inspect enough to document possible future integration points for review commands. The plan must make the next setup decision clear: what already exists, what new repo-local docs/commands are recommended, what optional tooling could help, what safe setup verification will run, and what requires separate approval. If the user approves the setup plan, apply only the approved repo-local/dev review commands/docs/tooling. Normalized review command setup must select the runner in this exact precedence: existing Just, existing Make, existing npm/package scripts, then add Just if no runner exists and the user approved it. Commands should cover sizing, static checks, dependency/architecture checks, tests, coverage, and a reasonable aggregate review command where appropriate. After applying approved changes, run each configured command when safe and classify each result as useful diagnostic signal, clean/successful signal, broken tooling, unavailable command, or intentionally skipped with reason; non-zero exits from discovered issues are useful diagnostic signal, not setup failure. CI and production dependency/config changes are forbidden for this setup workflow, as are required-check, branch-protection, and merge-blocker changes. Setup documentation must record the target repository's CI status and policy summary. If CI mutation or merge enforcement appears useful, ask for a separate explicit CI workflow instead of doing it.
