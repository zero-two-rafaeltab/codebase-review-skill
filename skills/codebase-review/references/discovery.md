...
1. **Restate the approved scope and cartography summary in your notes.** Include target paths or concerns, what is intentionally out of scope, the side-effect boundary, likely review slices, known gaps, and uncertainty.
2. **Choose a tiny component map from cartography.** Identify the main entry points, core modules, dependencies, data/configuration flows, tests, and documentation relevant to the scope without redoing broad mapping.
3. **Trace one or two representative paths.** Follow a typical request, command, job, data operation, or dependency usage through the code, selected from the likely review slices. For horizontal concerns, sample a few representative call sites instead.
...
8. **Keep additional delegation out of the main pass.** The required delegation for this phase is the cartography checkpoint. Do not design the later specialist plan, second go/no-go, or a complex delegation tree during this discovery pass.
